...
function forger_bestest()
	local count = 0
	local entity_id = GetUpdatedEntityID()
	local backup = EntityGetWithName("Extol'sBirdCounter")
	local other = EntityGetWithName("bhhgbmmklks")
	...
	if (entity_id ~= -1) and (run_count == false) then 
		...
		SetTimeOut( 1, "mods/Forge_Perks/files/scripts/alchemist_perk/worst_timeout.lua", "IHaveBeenBad" )
