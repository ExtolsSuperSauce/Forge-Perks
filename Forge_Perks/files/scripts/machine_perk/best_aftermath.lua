...
function forger_best()
	local count = 0
	local entity_id = GetUpdatedEntityID()
	local backup = EntityGetWithName("Extol'sMagicCounter")
	local other = EntityGetWithName("lbkshkvszvbjs")
	...
	if (entity_id ~= -1) and (run_count == false) then 
		...
		SetTimeOut( 1, "mods/Forge_Perks/files/scripts/alchemist_perk/best_timeout.lua", "IHaveBeenBad" )
